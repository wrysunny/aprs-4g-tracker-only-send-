...
local function proc(data)
    ...
    rdBuf = rdBuf .. data
    local result, unproc = false, rdBuf
    while true do
        result, unproc = parse(unproc)
        if not unproc or unproc == "" or not result then break end
    end
    rdBuf = unproc or ""
end
...
local function loadLib(keyword)
    ...
    if type(gpsLib)  == "table" and type(gpsLib.init)  == "function" then gpsLib.init()  end
    if type(agpsLib) == "table" and type(agpsLib.init) == "function" then agpsLib.init() end
end
...
sys.subscribe("GPS_WORK_ABNORMAL_IND", function()
    log.info("autoGPS.GPS_WORK_ABNORMAL_IND",
        not coSelfAdapt or coroutine.status(coSelfAdapt) == "dead")
    if not coSelfAdapt or coroutine.status(coSelfAdapt) == "dead" then
        os.remove(GPS_KIND_INFO_FILE)
        if type(gpsLib)  == "table" and type(gpsLib.unInit)  == "function" then gpsLib.unInit()  end
        if type(agpsLib) == "table" and type(agpsLib.unInit) == "function" then agpsLib.unInit() end
